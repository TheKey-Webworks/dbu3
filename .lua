...
local map = game.workspace.Others.Map;
local BossMaps = game.workspace.Others.Map;
...
task.spawn(function() -- Pick quest
    while ScGui and getloweststat() < checkplr()[3] do
        if Farming then
            ...
            -- eliminar efectos (modificar para volleys)

            local living = game.Workspace.Living

            for _, player in pairs(living:GetChildren()) do
                if player:FindFirstChild("Effects") then
                     player.Effects:Destroy()
                 end
            end
